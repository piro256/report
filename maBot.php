...
for ($i = 0; $i < mysqli_num_rows($ltpListRaw); $i++) {
    ...
    for ($q = 0; $q < count($ontSerialArray); $q++) {
        ...
        echo "$ontUniquelyId - slot - $ontSlotRaw<br>";
        //делаем серийный номер читабельным
        $ontSerial = ontSerial($ontSerialRaw);

        //запрашиваем слот
        $ontSlot = snmp2_get($ltpList[ip], "public", ".1.3.6.1.4.1.35265.1.22.3.1.1.1.{$ontSlotRaw}.8.69.76.84.88.{$ontUniquelyId}");

        //запрашиваем Channel
        $ontChannel = snmp2_get($ltpList[ip], "public", ".1.3.6.1.4.1.35265.1.22.3.1.1.3.{$ontSlotRaw}.8.69.76.84.88.{$ontUniquelyId}");

        //запрашиваем ID
        $ontId = snmp2_get($ltpList[ip], "public", ".1.3.6.1.4.1.35265.1.22.3.1.1.4.{$ontSlotRaw}.8.69.76.84.88.{$ontUniquelyId}");

        //запрашиваем стутус ONT
        $ontStateRaw = snmp2_get($ltpList[ip], "public", ".1.3.6.1.4.1.35265.1.22.3.1.1.5.{$ontSlotRaw}.8.69.76.84.88.{$ontUniquelyId}");
        $ontState = ontState($ontStateRaw);

        //запрашиваем rssi
        $ontRssi = str_replace("\"", "", snmp2_get($ltpList[ip], "public", ".1.3.6.1.4.1.35265.1.22.3.1.1.11.{$ontSlotRaw}.8.69.76.84.88.{$ontUniquelyId}")) * 0.1;

        //запрашиваем RxPower (не все отдают, смотри Rsii)
        $ontRxPower = snmp2_get($ltpList[ip], "public", ".1.3.6.1.4.1.35265.1.22.3.1.1.14.{$ontSlotRaw}.8.69.76.84.88.{$ontUniquelyId}") * 0.001;

        //Запращиваем Description
        $ontDescription = str_replace("\"", "", snmp2_get($ltpList[ip], "public", ".1.3.6.1.4.1.35265.1.22.3.4.1.8.{$ontSlotRaw}.8.69.76.84.88.{$ontUniquelyId}"));

        $updateTime = date("Y-m-j H:i:s");         

	    //Отнимает от слота 1 т.к. на оборудование нумерование идёт с 0
        --$ontSlot;
        //ищем ont в базе
